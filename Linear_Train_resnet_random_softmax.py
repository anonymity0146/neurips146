...
parser.add_argument("--outf", default="trainingOutput_random_resnet_softmax/",
                    help='folder to output images and model checkpoints')
parser.add_argument("--content_layers", default="conv3_5_c",
...
for iteration in range(1,opt.niter+1):
    ...
    ARCHITECTURE = 'resnet50_softmax_lastlayer'
        ...
        if ARCHITECTURE == 'resnet50_softmax_lastlayer':
            ...
            for name, layer in enumerate(model.layer3):
                ...
                if str(name) in layers3:
                    if layers3[str(name)] == 'conv3_5':
                        features['conv3_5_c'] = x
    ...
    if(iteration > 0 and (iteration) % opt.save_interval == 0):
        torch.save(matrix.state_dict(), '%s/%s_resnet_softmax_random.pth' % (opt.outf,opt.layer))
